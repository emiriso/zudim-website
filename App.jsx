...
const Pill = ({ children }) => (
  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-1 text-xs font-medium text-mutedfg">
    {children}
  </span>
)

const CTA = ({ href = '#contact', children }) => (
  <a href={href} className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 font-semibold text-fg shadow-sm hover:opacity-90">
    ...
    <div className="mx-auto max-w-6xl px-4">
      {(eyebrow || title) && (
        <div className="mb-10 md:mb-14">
          ...
          {title && <h2 className="mt-3 text-3xl font-bold leading-tight md:text-5xl">{title}</h2>}
          {subtitle && <p className="mt-4 max-w-3xl text-base text-mutedfg md:text-lg">{subtitle}</p>}
        </div>
      )}
      {children}
    </div>
...
const Feature = ({ icon: Icon, title, desc }) => (
  <motion.div initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true, amount:0.4}} transition={{duration:0.5}}
    className="group rounded-2xl border border-white/10 bg-obsidian/80 p-6 shadow-sm backdrop-blur-md hover:shadow-md">
    <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-mutedbg/40">
...
const PricingCard = ({ name, price, features, highlighted }) => (
  <div className={["rounded-2xl border p-6 shadow-sm border-white/10", highlighted ? "ring-2 ring-primary" : ""].join(" ")}>
    ...
    <div className="mt-4 flex items-end gap-1">
      ...
      <span className="text-sm text-mutedfg">/project</span>
    </div>
    ...
    <button className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2 font-medium text-fg hover:opacity-90">
...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-obsidian/80 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-6 md:flex">
          {links.map(l => (
            <a key={l.href} href={l.href} className="text-sm font-medium text-mutedfg hover:text-fg">{l.label}</a>
          ))}
          <CTA>Book a call</CTA>
        </nav>
        <a href="#contact" className="md:hidden"><Mail className="h-6 w-6" /></a>
      </div>
    </header>
  )
}
  ...
  return (
    <footer className="border-t border-white/10 py-10">
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm text-mutedfg">
              <li><a href="#about" className="hover:text-fg">About</a></li>
              <li><a href="#work" className="hover:text-fg">Case studies</a></li>
              <li><a href="#contact" className="hover:text-fg">Contact</a></li>
            </ul>
          </div>
          <div>
        ...
        <div className="mt-8 text-xs text-mutedfg">© {new Date().getFullYear()} Zudim Ltd. All rights reserved.</div>
      </div>
    </footer>
  )
}
...
function ContactForm() {
  ...
  return (
    <form onSubmit={(e)=>{e.preventDefault(); window.location.href = mailto;}}
          className="grid gap-4 rounded-2xl border border-white/10 bg-obsidian/60 p-6 shadow-sm backdrop-blur">
      <div className="grid gap-2">
        <label className="text-sm font-medium">Name</label>
        <input className="rounded-xl border border-white/10 bg-obsidian px-3 py-2 outline-none ring-primary focus:ring"
               value={name} onChange={(e)=>setName(e.target.value)} placeholder="Your name" required />
      </div>
      <div className="grid gap-2">
        <label className="text-sm font-medium">Email</label>
        <input type="email" className="rounded-xl border border-white/10 bg-obsidian px-3 py-2 outline-none ring-primary focus:ring"
               value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="you@company.com" required />
      </div>
      <div className="grid gap-2">
        <label className="text-sm font-medium">Message</label>
        <textarea className="min-h-[120px] rounded-xl border border-white/10 bg-obsidian px-3 py-2 outline-none ring-primary focus:ring"
                  value={message} onChange={(e)=>setMessage(e.target.value)} placeholder="Tell us about your project…" required />
      </div>
      <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-2 font-semibold text-fg hover:opacity-90">
...
export default function App() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-obsidian to-mutedbg/30 text-fg">
      <Header />
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div>
              ...
              <p className="mt-4 max-w-xl text-base text-mutedfg md:text-lg">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                <a href="#work" className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-5 py-3 font-semibold hover:bg-mutedbg/40">
                  View case studies
                </a>
              </div>
              <div className="mt-6 flex items-center gap-6 text-sm text-mutedfg">
            ...
            <motion.div initial={{opacity:0, y:16}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{duration:0.6}}
              className="rounded-3xl border border-white/10 bg-obsidian/70 p-4 shadow-[var(--shadow-xl)] backdrop-blur">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-white/10 p-4">
                  ...
                  <p className="mt-2 text-sm text-mutedfg">Ship a working pilot in weeks with measurable impact.</p>
                </div>
                <div className="rounded-2xl border border-white/10 p-4">
                  <div className="flex items-center gap-2">
                  ...
                  <p className="mt-2 text-sm text-mutedfg">Dashboards and reporting to track value and adoption.</p>
                </div>
                <div className="rounded-2xl border border-white/10 p-4">
                  <div className="flex items-center gap-2">
                  ...
                  <p className="mt-2 text-sm text-mutedfg">Security, privacy and change management built-in.</p>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {['AI Academy enablement', 'Nextcloud + n8n automations', 'Intelligent data pipeline'].map(item => (
            <motion.article key={item} initial={{opacity:0,y:10}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.45}}
                            className="group rounded-2xl border border-white/10 bg-obsidian/70 p-5 shadow-sm backdrop-blur hover:shadow-md">
              <div className="aspect-[16/10] w-full rounded-xl bg-gradient-to-br from-mutedbg to-mutedbg/40" />
      ...
      <Section id="about" eyebrow="Who we are" title="Bridging strategy, delivery and real-world adoption"
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-obsidian/60 p-6 shadow-sm">
      ...
      <Section id="contact" eyebrow="Let’s talk" title="Tell us about your project"
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 bg-obsidian/60 p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-mutedfg">We can do a 20-minute scoping chat to see if we’re a fit.</p>
            <div className="mt-4 flex flex-col gap-3 text-sm">
              <a href="tel:+447867165959" className="inline-flex items-center gap-2"><Phone className="h-4 w-4" /> +44 7867 165959</a>
              <a href="mailto:hello@zudim.co.uk" className="inline-flex items-center gap-2"><Mail className="h-4 w-4" /> hello@zudim.co.uk</a>
              <a href="#" className="inline-flex items-center gap-2"><Calendar className="h-4 w-4" /> Book a time</a>
            </div>
          </div>
        </div>
      </Section>
